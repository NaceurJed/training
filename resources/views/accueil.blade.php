...
@section('content')
    ...
    <header class="mb-4">
        <br>
        <br>
        <h1 class="text-light text-center" id="texte">En forme physiquement<br>et mentalement<br>grâce à un coaching
            personnalisé</h1>
        <a id="commencez" class="btn btn-dark btn-outline-light" data-bs-toggle="modal" href="#exampleModalToggle"
            role="button" tabindex="-1" aria-disabled="true">Commencez-maintenant</a>
                ...
                @foreach ($competitions as $competition)
                    <div class="carousel-item @if ($active) active @endif" data-bs-interval="5000">
                        ...
                        <div id="detailEvent" class="carousel-caption d-none d-md-block text-light">
                            ...
                            <a href="{{ route('affCompet', ['id' => $competition->id]) }}"
                                class="btn btn-outline-light">Info</a>
                        </div>
                    </div>
        ...
        @foreach ($articles as $article)
            <div class="card col-3 offset-1 mb-5 shadow-lg text-light" style="width: 22rem; background-color: black;">
                ...
                <div id="blocImgArticle">
                    <img src="{{ asset('Storage/' . $article->image) }}" id="imageCarte" class="card-img-top m-1 p-2"
                        alt="Footing">
                </div>
                <div class="card-body">
                    <h5 class="card-title text-center">{{ $article->titre }}</h5>
                    <p class="card-text">{{ substr($article->contenu, 0, 60) . '...' }}</p>
                    <p><i>Publié le {{ $article->created_at->format('d-m-Y') }} à
                            {{ $article->created_at->format('H:i') }}</i></p>
